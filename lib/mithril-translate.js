...
var mx_factory = function( m ) {
    ...
    function $load( languageToLoad ) {
        ...
        if( isBrowser( ) ) {
            ...
            var translations = require( $infix() + languageToLoad + $suffix() );
            currentLanguage = languageToLoad || currentLanguage;
            storage.set( translations );
            deferred.resolve();
        }
    ...
    /*
     * Returns if we are in a browser mode
     */
     function isBrowser( ) {
    ...
    /*
     * Returns if we are in a require compliant environment
     */
    function isServer( ) {
