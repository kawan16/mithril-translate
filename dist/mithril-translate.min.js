...
!function(m){"use strict";function $store(languageToUse,translationsToStore){currentLanguage=languageToUse||currentLanguage,storage.set(translationsToStore)}function $load(languageToLoad){m.request({method:"GET",url:$infix()+languageToLoad+$suffix()}).then(function(translations){currentLanguage=languageToLoad||currentLanguage,storage.set(translations)})}function $infix(){return configuration.infix}function $suffix(){return configuration.suffix||""}window.mx=window.mx||{};var configuration,currentLanguage,Storage=function(){var store={},variableRegex=/\{\{(.*?)\}\}/g;return{set:function(item){store[currentLanguage]=item},get:function(name,values){var translations=store[currentLanguage],result=translations;if(name.split(".").forEach(function(property){result=result[property]}),values){var variables=result.match(variableRegex);for(var key in values)variables.forEach(function(variable){-1!==variable.indexOf(key)&&(result=result.replace(variable,values[key]))})}return result}}},validators={string:function(string){if("string"!=typeof string)throw new TypeError("a string is expected, but "+string+" ["+typeof string+"] given");return string},plainObject:function(object){if("object"!=typeof object)throw new TypeError("an object is expected, but "+object+"  ["+typeof object+"] given");return object}},storage=new Storage;mx.translate=function(item){if(validators.string(item),arguments.length>1){var secondParameter=arguments[1];if(3===arguments.length){var thirdParameter=arguments[2];return storage.get(item+"."+thirdParameter,secondParameter)}return"object"==typeof secondParameter?storage.get(item,secondParameter):storage.get(item+"."+secondParameter)}return storage.get(item)},mx.translate.configure=function(options){validators.plainObject(options),configuration=options},mx.translate.use=function(languageToUse,optionalTranslations){return languageToUse&&currentLanguage!==languageToUse?(validators.string(languageToUse),void(optionalTranslations?$store(languageToUse,optionalTranslations):$load(languageToUse))):currentLanguage}}(m);
